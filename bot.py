...
@app.route('/healthz')
def healthz():
    """Render/Uptime liveness check.

    Keep this endpoint 200 while the process is alive.  Returning 503 when
    MongoDB has a transient network problem makes Render restart the bot,
    which is exactly when ephemeral local files are most at risk.
    """
    mongo_configured = bool(MONGODB_URI)
    mongo_ready = MONGO_DB is not None
    return {
        "status": "ok" if (not mongo_configured or mongo_ready) else "degraded",
...
MONGO_SYNC_INTERVAL_SECONDS = max(
    10, int(os.environ.get("MONGO_SYNC_INTERVAL_SECONDS", "10"))
)
...
def start_mongo_sync():
    ...
    # Create a current snapshot before polling starts.  Later snapshots run
    # every few seconds so an abrupt Render restart loses as little as possible.
    sync_sqlite_snapshot_to_mongo()
