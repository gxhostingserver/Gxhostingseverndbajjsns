...
except ImportError:  # The deployment requirements install these packages.
    MongoClient = None
    GridFSBucket = None
...
# --- Configuration ---
# All deployment-specific values come from environment variables.  Never put
# Telegram, Binance, or MongoDB credentials in this file or in GitHub.
...
# MongoDB is the durable store for Render's ephemeral filesystem.  The bot
# keeps its existing SQLite query layer for compatibility, and syncs the
# SQLite database plus uploaded source files to MongoDB.
...
def init_mongodb():
    """Connect to MongoDB when configured, without hiding local failures."""
...
def restore_sqlite_snapshot_from_mongo():
    ...
    if not MONGO_SNAPSHOT_META or not MONGO_FILES:
        return
...
def sync_hosted_file_to_mongo(user_id, file_path):
    ...
    if not MONGO_FILE_META or not MONGO_FILES or not os.path.isfile(file_path):
        return False
...
def sync_user_folder_to_mongo(user_id, user_folder):
    ...
    if not MONGO_FILE_META or not MONGO_FILES:
        return
...
def delete_hosted_file_from_mongo(user_id, file_name):
    """Remove the deleted source file from GridFS metadata and storage."""
    if not MONGO_FILE_META or not MONGO_FILES:
        return
...
def start_mongo_sync():
    """Keep the SQLite compatibility layer durable across Render restarts."""
    if not MONGO_SNAPSHOT_META or not MONGO_FILES:
        return
...
# Initialize persistence, DB, and in-memory data at startup.
init_mongodb()
